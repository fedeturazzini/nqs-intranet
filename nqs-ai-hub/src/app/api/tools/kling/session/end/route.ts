/**
 * POST /api/tools/kling/session/end
 *
 * Body: { sessionId: uuid, declaredConsumption: int ≥ 0 }
 * Response: { credits, creditsTotal, used }
 *
 * Cierra una sesión del módulo y, si el empleado declaró consumo, descuenta
 * créditos atómicamente vía el adapter (RPC consume_credit_atomic). Clon de
 * 3DSky.
 */
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (declaredConsumption > 0) {
    ...
    const result = await adapter.consumeCredit(
      session.userId,
      declaredConsumption,
      `Kling session ${sessionId.slice(0, 8)}`,
    );
    if (!result.ok) {
  ...
  // Cerrar la sesión. Si el consumo falló, guardamos 0 declarado.
  ...
  if (consumeError) {
    return NextResponse.json(
      { error: "consume_failed", message: consumeError },
      { status: 422 },
    );
  }
